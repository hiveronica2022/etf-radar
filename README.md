...
- 板块划分：**大盘宽基**（上证50/沪深300/中证500/中证1000/A500/深证100）与**创业科创**（创业板/科创板/双创）是两个独立板块——二者波动特征差异太大，混成一个"宽基"会掩盖大盘真实走势（如 2026 上半年大盘微跌、双创大涨）。
- 基准对照：分类走势图始终叠加一条**沪深300 指数**虚线（新浪公开数据），任何分组下都可与"官方大盘"直接对照。
- 分组下钻：走势图和对比图顶部的「分组」可从「板块」下钻到某板块的细分子类。例如红利细分为宽口径红利/红利低波/国企央企红利/港股红利，科技细分为半导体设备/科创芯片/芯片半导体/通信/人工智能/软件计算机/电子/互联网科技（半导体单独拆成设备/科创芯片/芯片半导体三类），大盘宽基/创业科创/债券同理。子类由 `classify_subcategory` 按名称关键词判定。
...
- 资金-价格象限：横轴区间涨跌（规模加权）、纵轴区间净流入，把每个板块落到追高/抄底/撤退/止盈四个象限，气泡大小为板块规模；跟随顶部「分组」可下钻到子类。
- 资金流环比：轮动图 1 周窗口下标注「较上周」变化（前一周 = 近2周 − 近1周），解读自动点名加速与降温板块。
- 避险温度计：0-100 风险偏好，=（价格风格分 + 资金偏好分）/2；风格分基于成长(创业科创+科技)与防御(红利+债券)的收益差 tanh 映射，资金分基于股类与债类净流入之差占比。两分背离 ≥40 时标「分歧」（如越跌越买）。实验性指标，仅供观察。
